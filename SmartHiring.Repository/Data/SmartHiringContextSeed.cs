...
{
	public static class SmartHiringContextSeed
	{
		public static async Task SeedAsync(SmartHiringContext dbContext)
		{
			if (!dbContext.Admins.Any())
			{
				var AdminsData = File.ReadAllText("../SmartHiring.Repository/Data/DataSeed/Admins.json");
			...
			{
				var ManagersData = File.ReadAllText("../SmartHiring.Repository/Data/DataSeed/Managers.json");
			...
			if (!dbContext.HRs.Any())
			{
				var HRsData = File.ReadAllText("../SmartHiring.Repository/Data/DataSeed/HRs.json");
				var HRs = JsonSerializer.Deserialize<List<HR>>(HRsData);
				if (HRs?.Count > 0)
				{
					foreach (var HR in HRs)
						await dbContext.Set<HR>().AddAsync(HR);
					await dbContext.SaveChangesAsync();
				}
			}
			...
			{
				var AgenciesData = File.ReadAllText("../SmartHiring.Repository/Data/DataSeed/Agencies.json");
				var Agencies = JsonSerializer.Deserialize<List<Agency>>(AgenciesData);
				if (Agencies?.Count > 0)
				{
					foreach (var Agency in Agencies)
						await dbContext.Set<Agency>().AddAsync(Agency);
					await dbContext.SaveChangesAsync();
				}
			}
